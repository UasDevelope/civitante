import 'package:civitante/App/utilse/widgets.dart';

class AppColors {
  static Color get appColor => Color(0xff000000);
  static Color get white => Color(0xffFFFFFF);
  static Color get textFieldHintColor => Color.fromRGBO(0, 0, 0, 0.7);
  static Color get textFiledBorderColor => Color.fromRGBO(255, 255, 255, 0.08);
  static Color get blue => Color(0xff2F89FC);
  static Color get moreblue => Color(0xff6552FE);
  static Color get Slate_gray => Color(0xff606A81);
  static Color get light_gray => Color(0xffD9D9D9);
  static Color get red_color => Color(0xffFF3B30);
  static Color get green => Color(0xff00B944);
}
